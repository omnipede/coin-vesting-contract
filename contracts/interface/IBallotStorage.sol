...
interface IBallotStorage {
    ...
    function createBallotForVariable(uint256, uint256, address, bytes32, uint256, string) external returns (uint256);
    ...
    function getBallotVariable(uint256) external view returns (bytes32, uint256, string);
}